...
// ─── QUICK ACTIONS ────────────────────────────────────────────────────────────
const QUICK = [
    { icon: "send",         label: "Envoyer",   color: C.orange, bg: "#FF7A3022", route: "/send"     },
    { icon: "call-received",         label: "Recevoir",  color: C.green,  bg: "#00D97E22", route: "/(tabs)/myqr"          },
    { icon: "credit-card",        label: "Recharger", color: C.blue,   bg: "#3B8BFF22", route: "/recharge" },
    { icon: "qr-code-scanner",    label: "Scanner",   color: C.purple, bg: "#B26EFF22", route: "/send"          },
];
 
// ─── SERVICES — chacun avec une couleur de fond dégradé ──────────────────────
const SERVICES = [
    { icon: "wifi",               label: "Internet",     color: "#3B8BFF", bg: "#0a1628", route:"/internet" },
    { icon: "phone-android",      label: "Crédit",       color: "#B26EFF", bg: "#160d28",route:"/internet" },
    { icon: "tv",                 label: "Canal+",       color: "#FF4559", bg: "#280d10",route:"/internet" },
    { icon: "bolt",               label: "E2C",         color: "#FFCC00", bg: "#1f1800" ,route:"/internet"},
    { icon: "water-drop",         label: "SNDE",         color: "#00C9B1", bg: "#001f1c",route:"/internet" },
    { icon: "school",             label: "Scolarité",    color: "#00D97E", bg: "#001f0f",route:"/internet" },
    { icon: "local-gas-station",  label: "Carburant",    color: "#FF7A30", bg: "#1f0f00",route:"/internet" },
    { icon: "more-horiz",         label: "Autres",       color: "#888888", bg: "#181818",route:"/internet" },
];
...
const BALANCES = [
    { label: "Airtime",  value: "0",   unit: "FCFA", icon: "phone",                  color: C.purple },
    { label: "Internet", value: "4.9", unit: "GB",   icon: "wifi",                   color: C.blue   },
    { label: "Voix",     value: "0",   unit: "Min",  icon: "mic",                    color: C.orange },
    { label: "MoMo",     value: null,  unit: "FCFA", icon: "account-balance-wallet", color: C.green  },
];
        ...
        <SafeAreaView style={s.safe}>
            ...
            >
                ...
                <View style={s.header}>
                    ...
                    <View style={s.headerRow}>
                        ...
                        <View style={s.headerIcons}>
                            <TouchableOpacity style={s.iconBtn}>
                                <MaterialIcons name="notifications-none" size={22} color={C.light} />
                    ...
                    {/* Balance card */}
                    <Animated.View style={[s.balCard, { transform: [{ scale: cardS }] }]}>
                        <Animated.View style={[s.glowOrb, { transform: [{ translateY: pulseY }] }]} />
 
                        <View style={s.balTop}>
                            <View style={{ flex: 1 }}>
                                <View style={s.balChip}>
                                    <View style={s.chipDot} />
                                    <Text style={s.chipTxt}>MTN MoMo</Text>
                                </View>
                                <Text style={s.balLbl}>Solde disponible</Text>
                                <View style={s.balAmtRow}>
                                    <Text style={s.balAmt}>
                                        {soldeVisible ? solde.toLocaleString("fr-FR") : "••••••"}
                                    </Text>
                                    <Text style={s.balCur}> FCFA</Text>
                                    <TouchableOpacity onPress={() => setSolde(!soldeVisible)} style={s.eyeBtn}>
                                        <MaterialIcons
                                ...
                                <Text style={s.balNum}>{user?.numero ? `+242 ${user.numero}` : "—"}</Text>
                            </View>
                            ...
                            {QUICK.map((q) => (
                                ...
                                >
                                    <View style={[s.quickIcon, { backgroundColor: q.bg }]}>
                                        <MaterialIcons name={q.icon as any} size={19} color={q.color} />
                                    </View>
                ...
                <Animated.View style={[s.content, { opacity: fade, transform: [{ translateY: slide }] }]}>
                    ...
                    {/* ══════════════════════════════════════════════════
                        SERVICES — GRILLE AVEC FOND COLORÉ
                    ══════════════════════════════════════════════════ */}
                    <View style={s.section}>
                        <View style={s.sectionHead}>
                            <View style={s.sectionTitleRow}>
                                <View style={[s.accent, { backgroundColor: C.blue }]} />
                                <Text style={s.sectionTitle}>Autres services</Text>
                            </View>
                            <TouchableOpacity style={s.seeAll} onPress={() => router.push("/payment")}>
                                <Text style={s.seeAllTxt}>Voir tout</Text>
                                <MaterialIcons name="arrow-forward-ios" size={10} color={C.yellow} />
                            </TouchableOpacity>
                        </View>
 
                        {/* Grille 4 colonnes avec fond coloré comme l'image */}
                        <View style={s.servGrid}>
                            {SERVICES.map((sv) => (
                                <TouchableOpacity key={sv.label} style={s.servItem} activeOpacity={0.75} onPress={() => router.push(sv.route as any)}
                                    >
                                    {/* Fond coloré de la case */}
                                    <View style={[s.servCard, { backgroundColor: sv.bg, borderColor: sv.color + "22" }]}>
                                        {/* Cercle déco en fond */}
                                        <View style={[s.servCircle, { backgroundColor: sv.color + "18" }]} />
                                        {/* Icône */}
                                        <View style={[s.servIconWrap, { backgroundColor: sv.color + "25" }]}>
                                            <MaterialIcons name={sv.icon as any} size={24} color={sv.color} />
                                        </View>
                                        {/* Label */}
                                        <Text style={[s.servLbl, { color: sv.color === "#FFCC00" ? C.yellow : C.light }]}>
                                            {sv.label}
                                        </Text>
                                    </View>
                                </TouchableOpacity>
                    ...
                    <View style={s.section}>
                        <View style={s.sectionHead}>
                            ...
                            <TouchableOpacity style={s.seeAll} onPress={() => router.push("/forfait")}>
                                <Text style={s.seeAllTxt}>Voir tout</Text>
                                <MaterialIcons name="arrow-forward-ios" size={10} color={C.yellow} />
                            </TouchableOpacity>
                        </View>
                    ...
                    <View style={s.section}>
                        <View style={s.sectionHead}>
                            <View style={s.sectionTitleRow}>
                                <View style={[s.accent, { backgroundColor: C.green }]} />
                                <Text style={s.sectionTitle}>Mes Soldes</Text>
                            </View>
                        </View>
                        ...
                        <View style={s.balGrid}>
                            {BALANCES.map((b) => {
                                const val = b.value === null
                                    ...
                                    : b.value;
                                return (
                                    <View key={b.label} style={s.balCell}>
                                        <View style={[s.balCellBg, { backgroundColor: b.color + "12" }]} />
                                        <View style={[s.balCellIcon, { backgroundColor: b.color + "20" }]}>
                                            <MaterialIcons name={b.icon as any} size={18} color={b.color} />
                                        </View>
                                        <Text style={s.balCellLbl}>{b.label}</Text>
                                        <Text style={s.balCellVal} numberOfLines={1}>{val}</Text>
                                        <Text style={s.balCellUnit}>{b.unit}</Text>
                                        <TouchableOpacity style={[s.balCellBtn, { borderColor: b.color + "50" }]}>
                                            <Text style={[s.balCellBtnTxt, { color: b.color }]}>+</Text>
                                        </TouchableOpacity>
                                    </View>
                                );
                            })}
                    ...
                    {/* ══════════════════════════════════════════════════
                        TRANSACTIONS
                    ══════════════════════════════════════════════════ */}
                    <View style={[s.section, { marginBottom: 110 }]}>
                        <View style={s.sectionHead}>
                            <View style={s.sectionTitleRow}>
                                <View style={[s.accent, { backgroundColor: C.orange }]} />
                                <Text style={s.sectionTitle}>Transactions récentes</Text>
                            </View>
                            <TouchableOpacity style={s.seeAll}>
                                <Text style={s.seeAllTxt}>Historique</Text>
                                <MaterialIcons name="arrow-forward-ios" size={10} color={C.yellow} />
                            </TouchableOpacity>
                        </View>
 
                        <View style={s.emptyTx}>
                            <View style={s.emptyTxCircle}>
                                <MaterialIcons name="receipt-long" size={32} color={C.muted} />
                            </View>
                            <Text style={s.emptyTxTitle}>Aucune transaction</Text>
                            <Text style={s.emptyTxSub}>Vos paiements et transferts apparaîtront ici</Text>
                            <TouchableOpacity style={s.emptyTxCta} onPress={() => router.push("/(tabs)/send" as any)}>
                                <MaterialIcons name="north-east" size={14} color={C.black} />
                                <Text style={s.emptyTxCtaTxt}>Faire un transfert</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
