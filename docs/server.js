...
db.connect((err) => {
  ...
  // Créer la table transferts si elle n'existe pas
  db.query(`
  ...
  // Créer la table paiements_factures
  db.query(`
    CREATE TABLE IF NOT EXISTS paiements_factures (
      id            INT AUTO_INCREMENT PRIMARY KEY,
      numero        VARCHAR(20)    NOT NULL,
      service       VARCHAR(50)    NOT NULL,
      ref_facture   VARCHAR(100)   NOT NULL,
      montant       DECIMAL(15,2)  NOT NULL,
      statut        VARCHAR(20)    NOT NULL DEFAULT 'succes',
      reference     VARCHAR(60)    NOT NULL UNIQUE,
      created_at    DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `, (err3) => {
    if (err3) console.error('Erreur création table paiements_factures:', err3.message);
    else console.log('Table paiements_factures prête');
  });
});
...
app.post('/api/transferts', (req, res) => {
  ...
  db.query('SELECT solde, name FROM users WHERE numero = ?', [fromClean], (err, senderRows) => {
    ...
    db.query('SELECT name, username, numero FROM users WHERE numero = ?', [toClean], (err2, destRows) => {
      ...
      // 3. Transaction atomique : débiter expéditeur + créditer destinataire + enregistrer
      db.beginTransaction((errTx) => {
        if (errTx) return res.status(500).json({ error: 'Erreur transaction' });
 
        // Débiter expéditeur
        db.query('UPDATE users SET solde = solde - ? WHERE numero = ?', [mt, fromClean], (e1) => {
          if (e1) return db.rollback(() => res.status(500).json({ error: 'Erreur débit' }));
 
          // Créditer destinataire
          db.query('UPDATE users SET solde = solde + ? WHERE numero = ?', [mt, toClean], (e2) => {
            if (e2) return db.rollback(() => res.status(500).json({ error: 'Erreur crédit' }));
            ...
            db.query(
              ...
              (e3) => {
                if (e3) {
                  ...
                  return db.rollback(() => res.status(500).json({ error: 'Erreur enregistrement: ' + e3.message }));
                }
 
                db.commit((eCom) => {
                  if (eCom) return db.rollback(() => res.status(500).json({ error: 'Erreur validation' }));
...
const genRefPaiement = () => 'PAY' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).substring(2, 5).toUpperCase();
 
// Catalogue des services et leurs tarifs fixes/variables
const SERVICES_CATALOGUE = {
  'SNDE': {
    label: 'SNDE - Eau',
    montants_fixes: [],          // montant libre
    montant_min: 500,
    montant_max: 500000,
  },
  'CANAL+': {
...
// ─── Vérifier une facture (simulé) ────────────────────────────────────────────
app.get('/api/paiements/verifier', (req, res) => {
  const { service, ref_facture } = req.query;
 
  if (!service || !ref_facture)
    return res.status(400).json({ error: 'Service et référence requis' });
 
  const catalogue = SERVICES_CATALOGUE[service.toUpperCase()];
  if (!catalogue)
    return res.status(404).json({ error: 'Service non reconnu' });
 
  if (!ref_facture.trim() || ref_facture.trim().length < 4)
    return res.status(400).json({ error: 'Référence trop courte (minimum 4 caractères)' });
 
  // Vérifier si une facture avec cette ref a déjà été payée (éviter double paiement)
  db.query(
    'SELECT id, created_at FROM paiements_factures WHERE service = ? AND ref_facture = ? AND statut = "succes"',
    [service.toUpperCase(), ref_facture.trim()],
    (err, rows) => {
      if (err) return res.status(500).json({ error: 'Erreur base de données' });
 
      if (rows.length > 0) {
        return res.status(400).json({
          error: `Cette facture ${service} a déjà été payée le ${new Date(rows[0].created_at).toLocaleDateString('fr-FR')}.`,
        });
      }
 
      // Simuler une vérification — en production, appeler l'API du fournisseur
      res.json({
        service: service.toUpperCase(),
        label: catalogue.label,
        ref_facture: ref_facture.trim(),
        montants_fixes: catalogue.montants_fixes,
        montant_min: catalogue.montant_min,
        montant_max: catalogue.montant_max,
        client: `Client ${ref_facture.trim().slice(-4).toUpperCase()}`,   // simulé
        valide: true,
      });
    }
  );
...
app.post('/api/paiements', (req, res) => {
  const { numero, service, ref_facture, montant } = req.body;
 
  if (!numero || !service || !ref_facture || !montant)
    return res.status(400).json({ error: 'Champs requis manquants' });
  ...
  const catalogue  = SERVICES_CATALOGUE[serviceKey];
  if (!catalogue)
    return res.status(404).json({ error: 'Service non reconnu' });
 
  // Vérifier double paiement
  db.query(
    'SELECT id FROM paiements_factures WHERE service = ? AND ref_facture = ? AND statut = "succes"',
    [serviceKey, ref_facture.trim()],
    (err0, existing) => {
      if (err0) return res.status(500).json({ error: 'Erreur vérification' });
      if (existing.length > 0)
        return res.status(400).json({ error: 'Cette facture a déjà été payée.' });
 
      // Vérifier le solde de l'expéditeur
      db.query('SELECT solde FROM users WHERE numero = ?', [numero], (err1, userRows) => {
        if (err1 || userRows.length === 0)
          return res.status(404).json({ error: 'Compte introuvable' });
 
        const solde = parseFloat(userRows[0].solde);
        if (solde < mt)
          return res.status(400).json({
            error: `Solde insuffisant. Disponible : ${solde.toLocaleString('fr-FR')} FCFA`,
          });
 
        const ref = genRefPaiement();
 
        // Transaction : débiter + enregistrer
        db.beginTransaction((errTx) => {
          if (errTx) return res.status(500).json({ error: 'Erreur transaction' });
 
          db.query('UPDATE users SET solde = solde - ? WHERE numero = ?', [mt, numero], (e1) => {
            if (e1) return db.rollback(() => res.status(500).json({ error: 'Erreur débit' }));
 
            db.query(
              'INSERT INTO paiements_factures (numero, service, ref_facture, montant, statut, reference) VALUES (?, ?, ?, ?, "succes", ?)',
              [numero, serviceKey, ref_facture.trim(), mt, ref],
              (e2) => {
                if (e2) {
                  console.error('Erreur INSERT paiement:', e2.message);
                  return db.rollback(() => res.status(500).json({ error: 'Erreur enregistrement: ' + e2.message }));
                }
 
                db.commit((eCom) => {
                  if (eCom) return db.rollback(() => res.status(500).json({ error: 'Erreur validation' }));
 
                  db.query('SELECT solde FROM users WHERE numero = ?', [numero], (e3, rows) => {
                    const nouveauSolde = rows?.[0] ? parseFloat(rows[0].solde) : solde - mt;
                    res.status(201).json({
...
app.get('/api/paiements', (req, res) => {
  ...
  db.query(
    'SELECT * FROM paiements_factures WHERE numero = ? ORDER BY created_at DESC LIMIT ?',
    [numero, parseInt(limit)],
    (err, results) => {
      if (err) return res.status(500).json({ error: 'Erreur base de données' });
      res.json(results);
